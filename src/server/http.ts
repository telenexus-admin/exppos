...
export async function tenantContext(req: NextRequest): Promise<TenantContext> {
  ...
  const token = header?.startsWith("Bearer ") ? header.slice(7) : req.cookies.get("tenant_session")?.value;
  ...
  try {
    ...
    return {
      ...
      branchIds: Array.isArray(payload.branchIds) ? payload.branchIds.filter((value): value is string => typeof value === "string") : [],
