...
export async function POST(req: NextRequest) {
  try {
    ...
    const user = await db.user.findFirst({ where: { tenant: { OR: [{ slug: businessKey.toLowerCase() }, { code: businessKey.toUpperCase() }], status: { in: ["TRIAL", "ACTIVE", "GRACE_PERIOD"] } }, OR: [{ email: body.identifier.toLowerCase() }, { phone: body.identifier }], status: "ACTIVE" }, include: { tenant: true, branches: true, roles: { include: { role: { include: { rolePermissions: { include: { permission: true } } } } } } } });
    ...
    if (!user || !valid) throw new AppError("INVALID_CREDENTIALS", "Incorrect business code, email/phone, or password", 401);
