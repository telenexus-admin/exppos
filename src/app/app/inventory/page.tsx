...
export default async function InventoryPage({
  ...
  const [viewer, tenant, branches, products, inventory] = await Promise.all([
    ...
    db.product.findMany({
      where: { tenantId: session.tenantId, status: "active" },
      select: { id: true, name: true, sku: true, trackStock: true, sellingPrice: true },
      orderBy: { name: "asc" },
    }),
        ...
        {filteredInventory.length === 0 ? (
          ...
          <div className="catalog-table-wrap">
            ...
            {filteredInventory.map((row) => {
              ...
              const rowContent = <><div className="catalog-product-cell"><span>{row.product.name.slice(0, 1).toUpperCase()}</span><div><strong>{row.product.name}</strong><small>{row.product.sku} · {row.product.category?.name ?? "Uncategorized"} · Selling {tenant.currency || "KES"} {Number(row.product.sellingPrice).toLocaleString("en-KE")}</small></div></div><div><strong>{row.branch.name}</strong><small>{row.branch.code}</small></div><div><strong>{isService ? "Unlimited" : quantity.toLocaleString("en-KE", { maximumFractionDigits: 3 })}</strong><small>{isService ? "Stock tracking disabled" : "Current balance"}</small></div><div><strong>{isService ? "—" : reorderLevel.toLocaleString("en-KE", { maximumFractionDigits: 3 })}</strong><small>Restock threshold</small></div><span className={`catalog-status ${statusClass}`}>{status}</span></>;
